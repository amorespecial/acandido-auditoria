  ...
  return (
    <div className="space-y-6" id="settings-tela">
      ...
      {activeTab === "USUARIOS" && (
        ...
        {/* PAINEL DE MIGRAÇÃO DE SENHAS (FASE 3) */}
        {user?.role === "ADMIN" && (
          <div className="bg-white rounded-xl border border-slate-100 shadow-xs p-5 space-y-4 mt-6">
            <div className="pb-2 border-b border-slate-150">
              <h3 className="text-sm font-black text-[#1B2A4A] uppercase tracking-wider flex items-center gap-2">
                <span className="material-symbols-outlined text-[18px] text-[#C8A84B]">security</span>
                Migração Segura de Senhas (Fase 3)
              </h3>
              <p className="text-[11.5px] text-slate-400 mt-0.5">
                Esta rotina converte as senhas em texto puro de usuários existentes para hashes seguros utilizando o algoritmo <strong>bcryptjs</strong>.
              </p>
            </div>

            <div className="text-xs text-slate-600 space-y-2 bg-slate-50/50 p-4 rounded-lg border border-slate-100">
              <p className="font-semibold text-[#1B2A4A]">Como funciona:</p>
              <ul className="list-disc pl-4 space-y-1">
                <li>Identifica todos os usuários cadastrados na tabela <code>usuarios</code> do Supabase.</li>
                <li>Ignora de forma idempotente qualquer usuário que já possua um hash <code>senha_hash</code> válido.</li>
                <li>Gera o hash com salt para os demais usuários, utilizando as credenciais cadastradas e salvas no sistema.</li>
                <li><strong>Garantia de Disponibilidade:</strong> Nenhuma informação atual de login é excluída ou alterada nesta fase. O login híbrido continua ativo para garantir que os usuários continuem acessando normalmente.</li>
              </ul>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <button
                ...
                className={`px-4 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider flex items-center gap-2 shadow active:scale-95 transition-all ${
                  isMigrating
                    ? "bg-slate-200 text-slate-450 cursor-not-allowed"
                    : "bg-[#1B2A4A] hover:bg-[#2A3E6B] text-white"
                }`}
              >
                {isMigrating ? (
                  <>
              ...
              {migrationResult && (
                <button
                  ...
                  className="px-3 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-500 rounded-lg text-xs font-bold uppercase tracking-wider"
                >
                  Limpar Relatório
                </button>
              )}
            ...
            {migrationError && (
              <div className="p-3 bg-red-50 border border-red-100 rounded-lg text-xs text-red-700 flex items-start gap-2">
                ...
                <div>
                  <span className="font-bold">Erro de execução:</span> {migrationError}
                </div>
              </div>
            )}
              ...
              <div className="space-y-3 pt-2">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-slate-50/50 border border-slate-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-[#1B2A4A]">{migrationResult.totalFound}</div>
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Encontrados</div>
                  </div>
                  <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-emerald-700">{migrationResult.migratedCount}</div>
                    <div className="text-[10px] text-emerald-500 font-bold uppercase tracking-wider">Migrados</div>
                  </div>
                  <div className="bg-blue-50/50 border border-blue-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-blue-700">{migrationResult.ignoredCount}</div>
                    <div className="text-[10px] text-blue-500 font-bold uppercase tracking-wider">Ignorados</div>
                  </div>
                  <div className="bg-red-50/50 border border-red-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-red-700">{migrationResult.errorCount}</div>
                    <div className="text-[10px] text-red-500 font-bold uppercase tracking-wider">Erros</div>
                  </div>
                </div>

                {migrationResult.errors.length > 0 && (
                  <div className="bg-red-50/30 border border-red-100 rounded-lg p-3 space-y-1">
                    <div className="text-[11px] font-bold text-red-800 uppercase tracking-wider">Detalhes das Falhas:</div>
                    <div className="max-h-40 overflow-y-auto font-mono text-[10.5px] text-red-600 space-y-1 pl-4 list-disc">
                ...
                {migrationResult.errors.length === 0 && migrationResult.migratedCount > 0 && (
                  <div className="p-3 bg-emerald-50/40 border border-emerald-100 rounded-lg text-xs text-emerald-800 flex items-center gap-1.5 font-medium">
                    <span className="material-symbols-outlined text-[16px]">check_circle</span>
                    Migração finalizada sem erros! Todos os usuários elegíveis foram migrados para senha_hash com sucesso.
                  </div>
                )}

                {migrationResult.errors.length === 0 && migrationResult.migratedCount === 0 && (
                  <div className="p-3 bg-blue-50/40 border border-blue-100 rounded-lg text-xs text-blue-800 flex items-center gap-1.5 font-medium">
                    <span className="material-symbols-outlined text-[16px]">info</span>
                    Todos os usuários já estavam migrados (senha_hash presente). Nenhuma ação foi necessária.
                  </div>
                )}
              </div>
            )}
        ...
        {/* PAINEL DE LIMPEZA DEFINITIVA DE SENHAS (FASE 4) */}
        {user?.role === "ADMIN" && (
          <div className="bg-white rounded-xl border border-slate-100 shadow-xs p-5 space-y-4 mt-6">
            <div className="pb-2 border-b border-slate-150">
              <h3 className="text-sm font-black text-[#1B2A4A] uppercase tracking-wider flex items-center gap-2">
                <span className="material-symbols-outlined text-[18px] text-red-600">cleaning_services</span>
                Limpeza Definitiva das Senhas Legadas (Fase 4.2)
              </h3>
              <p className="text-[11.5px] text-slate-400 mt-0.5">
                Esta rotina remove de forma definitiva as senhas em texto puro armazenadas no campo JSON dos usuários que já possuem um hash seguro de senha cadastrado.
              </p>
            </div>

            <div className="text-xs text-slate-600 space-y-2 bg-slate-50/50 p-4 rounded-lg border border-slate-100">
              <p className="font-semibold text-[#1B2A4A]">O que esta ação faz:</p>
              <ul className="list-disc pl-4 space-y-1">
                <li>Percorre todos os usuários na base do Supabase.</li>
                <li>Verifica se o usuário possui a coluna <code>senha_hash</code> preenchida.</li>
                <li>Caso possua, limpa com segurança a propriedade <code>password</code> dentro do JSON da coluna <code>almoxarifado</code>.</li>
                <li>Mantém intactas todas as demais configurações do JSON (cargo, grupo, almoxarifados permitidos, etc.).</li>
                <li><strong>Garantia de Segurança:</strong> A partir deste momento, nenhum usuário terá senhas em texto puro salvas no sistema e o login utilizará estritamente a senha criptografada.</li>
              </ul>
            </div>
              ...
              >
                {isCleaning ? (
                  ...
                  <>
                    ...
                    Executar Limpeza de Senhas Legadas
                  </>
                )}
              </button>
            ...
            {cleanupError && (
              <div className="p-3 bg-red-50 border border-red-100 rounded-lg text-xs text-red-700 flex items-start gap-2">
                <span className="material-symbols-outlined text-[16px] mt-0.5">error</span>
                <div>
                  <span className="font-bold">Erro de execução:</span> {cleanupError}
                </div>
              </div>
            )}
            ...
            {cleanupResult && (
              <div className="space-y-3 pt-2">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-slate-50/50 border border-slate-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-[#1B2A4A]">{cleanupResult.totalUsers}</div>
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Usuários Totais</div>
                  </div>
                  <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-emerald-700">{cleanupResult.cleanedCount}</div>
                    <div className="text-[10px] text-emerald-500 font-bold uppercase tracking-wider">Limpos Nesta Execução</div>
                  </div>
                  <div className="bg-blue-50/50 border border-blue-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-blue-700">{cleanupResult.alreadyCleanedCount}</div>
                    <div className="text-[10px] text-blue-500 font-bold uppercase tracking-wider">Já Estavam Limpos</div>
                  </div>
                  <div className="bg-red-50/50 border border-red-100 p-3 rounded-lg text-center">
                    <div className="text-lg font-black text-red-700">{cleanupResult.errorCount}</div>
                    <div className="text-[10px] text-red-500 font-bold uppercase tracking-wider">Erros / Alertas</div>
                  </div>
                </div>

                {cleanupResult.errors.length > 0 && (
                  <div className="bg-red-50/30 border border-red-100 rounded-lg p-3 space-y-1">
                    <div className="text-[11px] font-bold text-red-800 uppercase tracking-wider">Falhas ou Alertas Identificados:</div>
                    <div className="max-h-40 overflow-y-auto font-mono text-[10.5px] text-red-600 space-y-1 pl-4 list-disc">
                ...
                {cleanupResult.errors.length === 0 && cleanupResult.cleanedCount > 0 && (
                  <div className="p-3 bg-emerald-50/40 border border-emerald-100 rounded-lg text-xs text-emerald-800 flex items-center gap-1.5 font-medium">
                    <span className="material-symbols-outlined text-[16px]">check_circle</span>
                    Limpeza concluída com sucesso! Os dados de senhas legadas foram apagados das contas migradas.
                  </div>
                )}
                ...
                {cleanupResult.errors.length === 0 && cleanupResult.cleanedCount === 0 && (
                  <div className="p-3 bg-blue-50/40 border border-blue-100 rounded-lg text-xs text-blue-800 flex items-center gap-1.5 font-medium">
                    <span className="material-symbols-outlined text-[16px]">info</span>
                    Todos os usuários qualificados já estavam totalmente livres de senhas legadas.
                  </div>
                )}
              </div>
            )}
