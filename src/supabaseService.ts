...
export const dbFetchSchedules = async (): Promise<any[]> => {
  ...
  const { data, error } = await supabase.from('calendario_inventarios').select('*').order('sequencia', { ascending: true });
  if (error || !data) {
...
export const dbSaveSchedules = async (schedules: any[], forceYear?: number) => {
  ...
  try {
    ...
    // 1. Determine unique combinations of (almoxarifado_id, semestre, ano) to delete and insert
    // We will group items by key: almoxarifado_id_semestre_ano
    const groups = new Map<string, { almoxarifado_id: string; semestre: number; ano: number; items: any[] }>();

    // If forceYear is provided, we can pre-populate groups for all branchIds present in the schedules for that year
    if (forceYear) {
      const branchIds = Array.from(new Set(schedules.map(s => s.branchId || s.almoxarifado_id).filter(Boolean)));
      for (const bId of branchIds) {
        for (const sem of [1, 2]) {
          const key = `${bId}_${sem}_${forceYear}`;
          groups.set(key, { almoxarifado_id: bId, semestre: sem, ano: forceYear, items: [] });
        }
      }
    }
    ...
    for (const item of schedules) {
      const almoxId = item.branchId || item.almoxarifado_id;
      if (!almoxId) continue;
      ...
      if (!groups.has(key)) {
        groups.set(key, { almoxarifado_id: almoxId, semestre: sem, ano: year, items: [] });
      }

      // Only add items that have a valid date_agendada or status/link update
    ...
    for (const [key, group] of groups.entries()) {
      console.log(`[dbSaveSchedules] Deleting existing records for - AlmoxId: ${group.almoxarifado_id}, Semestre: ${group.semestre}, Ano: ${group.ano}`);
      
      // Execute DELETE
      const { error: deleteError } = await supabase
        .from('calendario_inventarios')
        .delete()
        .eq('almoxarifado_id', group.almoxarifado_id)
        .eq('semestre', group.semestre)
        .eq('ano', group.ano);

      if (deleteError) {
        console.error("Delete error in dbSaveSchedules:", deleteError);
        throw new Error(`Falha ao limpar agendamentos antigos para ${group.almoxarifado_id}: ${deleteError.message}`);
      }
      ...
      if (group.items.length > 0) {
        const recordsToInsert = group.items.map((item, idx) => {
          ...
          const generatedId = `cal-${group.almoxarifado_id}-${group.ano}-${group.semestre}-${idxVal}`;
          
          console.log(`[dbSaveSchedules] PREPARING INSERT - ID: ${generatedId}, almoxarifado_id: ${group.almoxarifado_id}, DATA: ${item.data_agendada}`);

          return {
            id: generatedId,
            almoxarifado_id: group.almoxarifado_id,
        ...
        if (insertError) {
          ...
          throw new Error(`Falha ao inserir novos agendamentos para ${group.almoxarifado_id}: ${insertError.message}`);
        }
        ...
        if (confirmError) {
          ...
          throw new Error(`Confirmação falhou para ${group.almoxarifado_id}: Dados salvos não correspondem ao esperado.`);
        } else {
          console.log(`[dbSaveSchedules] SUCCESS - Confirmed ${confirmData.length} records in DB for:`, group.almoxarifado_id, confirmData);
        }
      } else {
        console.log(`[dbSaveSchedules] Clear done (0 new schedules) for - AlmoxId: ${group.almoxarifado_id}, Semestre: ${group.semestre}, Ano: ${group.ano}`);
      }
    }
  } catch (err) {
...
export const dbFetchBranchSchedules = async (branchId: string, options?: { ano?: number; semestre?: number }): Promise<any[]> => {
  ...
  try {
    ...
    query = query.order('sequencia', { ascending: true });
    ...
    console.log(`[dbFetchBranchSchedules] READ SUCCESS - Almoxarifado ID: ${branchId}, count: ${data.length}`, data);
